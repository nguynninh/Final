...
void exe() {
    //char pass_ano[3];
    unsigned int input_times = 0, input = -1, pass = 0, require = 108, reader;
...
//        require = reader;   
    ...
    while (pass != require) {
        ...
        while (input_times != 3) {
            ...
            if (input != -1) {
                input_times++;
                pass = pass * 10 + input;
                mainScreen2("Nhap pass?", pass_ano, 30);
                mainScreen3("Nhap pass?", input, 30+(input_times-1)*4+input_times, pass_ano);
    ...
    glcd_clear();
    PORTC.3 = 0;
    glcd_outtextxy(15, 20, "pass dung"); 
}
