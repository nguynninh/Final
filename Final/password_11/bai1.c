...
void exe() {
    ...
    while (pass != require) {
        ...
        pass_ano[1]=' ';
        pass_ano[2]=' ';
        mainScreen("PASSWORD?", -1, sai_toi_da-dem_sai);
        while (input_times != 3) {
            input = read();
            delay_ms(200);
            if (input != -1) {
                input_times++;
                pass = pass * 10 + input;
                mainScreen2("PASSWORD?", pass_ano, 30, sai_toi_da-dem_sai);
                mainScreen3("PASSWORD?", input, 30+(input_times-1)*4+input_times, pass_ano, sai_toi_da-dem_sai);
        ...
        if (dem_sai == sai_toi_da){
            ...
            PORTC.3 = 1;
            glcd_clear();
            glcd_outtextxy(15, 5, "khoa thiet bi");
            return;
        }
        //miss++;
        // turn on role 1
...
void main(void)
{
    ...
    #asm("sei")
    update_eeprom_word(&my_eeprom, 118);
    exe();
    delay_ms(1500);
